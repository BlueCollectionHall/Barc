# Barc 蔚蓝收录馆 - Agent 行为约束文档（宪法）

> 本文档用于约束所有 Agent 智能体在本项目中的行为规范。
> 所有 Agent 在工作前必须阅读并遵守本文档。
>
> **标准宪法文件说明：**
> - 本文件 `AGENTS.md` 为项目**标准宪法文件**，DeepSeek Harness（DSH）读取的正是本文件（DSH 的指令文件候选为 `AGENTS.md`、`CLAUDE.md`，不读取 `AGENT.md`）。
> - 历史文件 `AGENT.md`（单数）仅作为兼容别名，其内容为指向本文件的引用；**未来所有宪法修改一律在本文件 `AGENTS.md` 上进行**，每次修改后需同步更新 `AGENT.md` 的引用说明。
> - 所有 Agent 须以本文件为唯一标准，除非明确说明，否则忽略其他同名文档。
...
- **思考原则**：全程使用中文进行思考（包括问题拆解、推理过程、方案权衡、边界判断等内部推理）
- **测试要求**：每次修改必须测试
- **审查要求**：每次修改必须经过审查
